...
hc05_lines = []
with open ('I:\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\StudyCenter\\SS_Arduino.txt', 'r') as myfile: 
    for hc05_line in myfile:                
        hc05_lines.append(hc05_line)           

esp_lines = []
with open ('I:\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\StudyCenter\\SS_Esp.txt', 'r') as myfile: 
...
sniff_start_time = datetime.strptime("22/4/2022 13:39:40", "%d/%m/%Y %H:%M:%S") #this time is for ss sniff data
phase_a_end_time = datetime.strptime("22/4/2022 13:44:40", "%d/%m/%Y %H:%M:%S")
phase_b_end_time = datetime.strptime("22/4/2022 13:49:40", "%d/%m/%Y %H:%M:%S")
phase_c_end_time = datetime.strptime("22/4/2022 13:54:40", "%d/%m/%Y %H:%M:%S")
sniff_stop_time = datetime.strptime("22/4/2022 13:56:46", "%d/%m/%Y %H:%M:%S") #this time is for ss sniff data
...
for mac in hc_05_mac_addresses_list:
    ...
    for counter_of_time in range(hc05_amount_of_date_times):
        HC05_recorded_time = hc05_date_time[counter_of_time]
            ...
            #print(recorded_time)
        if  HC05_recorded_time >= sniff_start_time and HC05_recorded_time < phase_a_end_time:    
        ...
        if  HC05_recorded_time >= phase_a_end_time and HC05_recorded_time < phase_b_end_time:     
        ...
        if  HC05_recorded_time >= phase_b_end_time and HC05_recorded_time < phase_c_end_time:        
...
plt.xticks([r + (barWidth/2) for r in range(len(Hc05_height))],
        ['13:39:40 - 13:44:39', '13:44:40 - 13:49:39', '13:39:40 - 13:56:46'])
...
#plt.legend()
plt.title("Periodic Total Devices Detected At Engineering Study Centre", fontweight = 'bold', fontsize = 30)
plt.show()