...
hc05_lines = []
with open ('I:\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\Neelsie\\Neelsie_Arduino.txt', 'r') as myfile: 
    for hc05_line in myfile:                
        hc05_lines.append(hc05_line)           

esp_lines = []
with open ('I:\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\Neelsie\\Neelsie_Esp.txt', 'r') as myfile: 
...
# the following is for the neelsie
sniff_start_time = datetime.strptime("25/4/2022 12:26:00", "%d/%m/%Y %H:%M:%S") #this time is for ss sniff data
sniff_stop_time = datetime.strptime("25/4/2022 12:47:00", "%d/%m/%Y %H:%M:%S") #this time is for ss sniff data
phase_a_end_time = datetime.strptime("25/4/2022 12:31:00", "%d/%m/%Y %H:%M:%S")
phase_b_end_time = datetime.strptime("25/4/2022 12:36:00", "%d/%m/%Y %H:%M:%S")
phase_c_end_time = datetime.strptime("25/4/2022 12:41:00", "%d/%m/%Y %H:%M:%S")
phase_d_end_time = datetime.strptime("25/4/2022 12:46:00", "%d/%m/%Y %H:%M:%S")
...
HC05_macs_sniffed_phase_d = [] #only for  neelsie
...
for mac in hc_05_mac_addresses_list:
    ...
    for counter_of_time in range(hc05_amount_of_date_times):
        ...
        # this is for the neelsie sniff which was 20 minutes and hence 4 periods
        if  phase_c_end_time <= HC05_recorded_time and phase_d_end_time >= HC05_recorded_time:
            if hc05_one_addresses[0] not in HC05_macs_sniffed_phase_d:
                HC05_macs_sniffed_phase_d.append(hc05_one_addresses[0])                
...
print("The HC05 total devices for phase D is:",end=" ") 
#print(len(HC05_macs_sniffed_phase_d)) 

ESP_macs_sniffed_phase_a = []
ESP_macs_sniffed_phase_b = []
ESP_macs_sniffed_phase_c = []
ESP_macs_sniffed_phase_d = [] #only for  neelsie
...
for mac in esp_mac_addresses_list:
    ...
    for ESP_counter_of_time in range(esp_amount_of_date_times):
        ...
        # this is for the neelsie sniff which was 20 minutes and hence 4 periods
        if  phase_c_end_time <= ESP_recorded_time and phase_d_end_time >= ESP_recorded_time:
            if esp_one_addresses[0] not in ESP_macs_sniffed_phase_d:
                ESP_macs_sniffed_phase_d.append(esp_one_addresses[0]) 
...
print("The ESP total devices for phase D is:",end=" ") 
#print(len(ESP_macs_sniffed_phase_d)) 

# the following is for Neelsie sniff
Hc05_height = [(len(HC05_macs_sniffed_phase_a)-2), (len(HC05_macs_sniffed_phase_b)-2), (len(HC05_macs_sniffed_phase_c)-2), (len(HC05_macs_sniffed_phase_d)-2)] #array is 3 except for neelsie data
Esp_height = [(len(ESP_macs_sniffed_phase_a)-2), (len(ESP_macs_sniffed_phase_b)-2), (len(ESP_macs_sniffed_phase_c)-2), (len(ESP_macs_sniffed_phase_d)-2)] #array is 3 except for neelsie data

# Hc05_height = [len(HC05_macs_sniffed_phase_a), len(HC05_macs_sniffed_phase_b), len(HC05_macs_sniffed_phase_c)] #array is 3 except for neelsie data
# Esp_height = [len(ESP_macs_sniffed_phase_a), len(ESP_macs_sniffed_phase_b), len(ESP_macs_sniffed_phase_c)] #array is 3 except for neelsie data

control_devices = [2, 2, 2, 2] #Array size is 3 except for neelsie data
...
# plt.bar(bar1, Hc05_height, color='dodgerblue', width = barWidth, edgecolor='grey', label='HC05')
# plt.bar(bar2, Esp_height, color ='orangered', width = barWidth, edgecolor ='grey', label ='ESP32')
...
# the following is for neelsie data
plt.xticks([r + (barWidth/2) for r in range(len(Hc05_height))],
        ['12:26:00 - 12:30:59', '12:31:00 - 12:35:59', '12:36:00 - 12:40:59', '12:41:00 - 12:45:59'])
...
#plt.legend()
plt.title("Periodic Total Devices Detected At The Neelsie Student Center", fontweight = 'bold', fontsize = 30)
plt.show()