...
with open ('I:\\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\Chalkies\\Chalkboard_Arduino.txt', 'r') as myfile: # Open lorem.txt for reading text data.
                ...
                if i==j:
                    ...
                    if( minutes_diff <= 0.55):
                        ...
                        if t1 not in ploted_time and t2 not in ploted_time:
                            ...
                            plt.plot(t1, mac,marker='o',color='dodgerblue') #this color is for HC-05 

                        elif t2 not in ploted_time:
                            plt.plot(t2, mac,marker='o',color='dodgerblue') #this color is for HC-05 
...
custom_lines = [Line2D([0], [0],marker='o', color='w',markerfacecolor='black', markersize=15),
                Line2D([0], [0],marker='o', color='w',markerfacecolor='dodgerblue', markersize=15),
                Line2D([0], [0],marker='o', color='w',markerfacecolor='limegreen', markersize=15)]
first_legend = plt.legend(custom_lines,['Device detected once','Device detected more than once','Control device'],fontsize=15)
...
plt.legend(handles=[solid,dashed],  bbox_to_anchor=(0,0.8), loc='lower left',fontsize=15)
#plt.legend(handles=[solid,dashed],loc='upper left',prop={'size': 15})
plt.xlabel('Time', fontweight ='bold', fontsize = 30)
plt.ylabel('Devices found', fontweight ='bold', fontsize = 30)
plt.title("HC-05 At Engineering Student Cafe Public Location", fontweight = 'bold', fontsize = 40)
plt.show()