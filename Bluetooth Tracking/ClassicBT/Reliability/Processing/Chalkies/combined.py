...
with open ('I:\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\Chalkies\\Chalkboard_Arduino.txt', 'r') as myfile1: # Open lorem.txt for reading text data.  #I:\Other computers\My MacBook Air\2_CBT_DEVICES\Processing\StudyCenter
...
with open ('I:\\Other computers\\My MacBook Air\\2_CBT_DEVICES\\Processing\\Chalkies\\Chalkboard_Esp.txt', 'r') as myfile2: 
...
#     print()
#     print(mac2)
#     for mac22 in mac_addresses2:
#         if (mac22 == mac2): #we want to make sure we don't skip any mac since the list has been sorted so we go using the old list
#             for line2 in mylines2:
#                 data2 = json.loads(line2)
...
for (i_hc_mac,i_esp_mac) in itertools.zip_longest(sorted_hc_macs, sorted_esp_macs):
    if(count1 <= 11): #hard coding because the hc05 picked 10 macs 
                ...
                for j in range(length_of_times):
                    ...
                    if i==j:
                        ...
                        if( minutes_diff <= 0.55):
                            ...
                            if(i_hc_mac == 'E8:DB:84:3B:3A:5A' or i_hc_mac == 'E8:DB:84:3D:EF:9A'):
                            ...
                            if t1 not in ploted_time and t2 not in ploted_time:
                                if(i_hc_mac == 'E8:DB:84:3B:3A:5A' or i_hc_mac == 'E8:DB:84:3D:EF:9A'):
                                    plt.plot(t, m,marker='o',linestyle = '--',color='limegreen') #this color is for HC-05 
                                    t=[]
                                    m=[]
                                else:    
            ...
            for i in range(length_of_times2):
                ...
                for j in range(length_of_times2):
                    ...
                    if i==j:
                        ...
                        if( minutes_diff <= 0.55):
                            #if(mac == 'e8:db:84:3b:3a:5a' or mac == 'e8:db:84:3d:ef:9a'): #this is for esp
                            if(i_esp_mac == 'e8:db:84:3b:3a:5a' or i_esp_mac == 'e8:db:84:3d:ef:9a'):
                                #print("Time within contious plot window")
                                ploted_time.append(t1)
                                ploted_time.append(t2)
                                plt.plot(t, m,marker='o',color='limegreen') #this is for esp color
...
ti_cks = [1.5, 3.5, 5.5, 7.5, 9.5, 11.5, 13.5, 15.5, 17.5, 19.5] 
...
custom_lines = [Line2D([0], [0],marker='o', color='w',markerfacecolor='dodgerblue', markersize=20),
                Line2D([0], [0],marker='o', color='w',markerfacecolor='orangered', markersize=20),
                Line2D([0], [0],marker='o', color='w',markerfacecolor='limegreen', markersize=20)]
#first_legend = plt.legend(custom_lines,['HC05 detected device','ESP32 detected device','Control device (ESP32 in classic BT mode)'],bbox_to_anchor=(0.005,0.6089),loc='lower left',fontsize = 15)
first_legend = plt.legend(custom_lines,['HC05 detected device','ESP32 detected device','Control device (ESP32 in classic BT mode)'],bbox_to_anchor=(0.5,0.5),fontsize = 15)
ax = plt.gca().add_artist(first_legend)
#from matplotlib.legend import Legend


import matplotlib.lines as mlines
solid = mlines.Line2D([], [], color='black', linestyle='-',markersize=20, label='Solid line: Device was always detected')
dashed = mlines.Line2D([], [], color='black', linestyle='--',markersize=20, label='Dashed line: Device not always detected')
plt.legend(handles=[solid,dashed],  bbox_to_anchor=(0.005,0.8), loc='upper left',fontsize = 15)

plt.xticks(fontsize=20)
plt.yticks(fontsize=12)
plt.xlabel('Time', fontweight ='bold', fontsize = 30)
plt.ylabel('Devices found', fontweight ='bold', fontsize = 30)
plt.title("HC-05 & ESP32 At Engineering Student Cafe", fontweight = 'bold', fontsize = 35)
plt.show()